const queries = [
  { query: "Gasto total agrupado por ano", id: "query1" },
  {
    query: "Gasto total agrupado por ano e órgão responsável",
    id: "query2",
  },
  { query: "Gasto total agrupado por presidente", id: "query3" },
  {
    query: "Gastos de cartões de pagamento agrupado por ano",
    id: "query4",
  },
  { query: "Total de despesas agrupado por ano", id: "query5" },
  { query: "Total de licitações agrupado por ano", id: "query6" },
  {
    query: "Total de licitações agrupado por órgão responsável",
    id: "query7",
  },
  { query: "Gastos com viagens agrupado por ano", id: "query8" },
  {
    query: "Gastos com viagens agrupado por órgão responsável",
    id: "query9",
  },
  {
    query: "Ranking das viagens mais caras agrupadas por órgão responsável",
    id: "query11",
  },
  {
    query: "Média dos valores das diárias em viagens agrupado por ano",
    id: "query12",
  },
  {
    query: "Ranking dos itens mais comuns comprados em licitações",
    id: "query13",
  },
  {
    query: "Ranking dos favorecidos nas compras por cartão de pagamento",
    id: "query14",
  },
  {
    query: "Ranking das funções (Ministérios) com maior número de gastos",
    id: "query15",
  },
  {
    query: "Ranking de portadores com mais gastos no cartao de pagamento",
    id: "query16",
  },
  {
    query:
      "Funções de orçamentaria com maiores gastos agrupados por órgao responsável",
    id: "query17",
  },
  {
    query: "Funções de orçamentaria com maiores gastos médios anuais",
    id: "query18",
  },
  {
    query: "Subfunções de orçamentaria com maiores gastos médios anuais",
    id: "query19",
  },
];

export default queries;
